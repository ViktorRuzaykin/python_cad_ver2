...
class FileCsv:
    ...
    def create_base_file(self):
        ...
        """
        time.sleep(1)
        project_marks_pipe = self.import_data.import_project_marks('Выдели отметки по трубе')
        project_marks_earth = self.import_data.import_project_marks('Выдели отметки по земле')
        project_distance = self.import_data.import_distance_line('Выдели отрезки расстояний')
        ...
        name_file_base = f'ПК{self.start_picket[0]}+{"{:.2f}".format(float(self.start_picket[0]))} ' \
...
"""import_cad = FileCsv([0, 0])
# import_cad.create_base_file()
name_file = 'ПК0+0.00 - ПК5+0.00_base.csv'
import_cad.create_final_file(name_file)
"""
