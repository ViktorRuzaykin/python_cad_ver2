...
def calc_file():
    ...
    f_mark_earth.clear()
    open_file_1 = askopenfilename()
    # обработка CSV файла
    with open(open_file_1) as r_file:
        file_reader = csv.reader(r_file, delimiter=";")
        for row in file_reader:
            f_pk_int.append(int(row[0]))
            f_pk_float.append(row[1])
            f_mark_pipe.append(float(row[2]))
            f_mark_earth.append(float(row[3]))
            f_mark_ditch.append(float(row[2]) - 1.62)
            f_mark_pillow.append(float(row[2]) - 1.42)
            f_mark_filling.append(float(row[2]) + 0.20)
    ...
    new_file_calc = f'{int(f_pk_int[0])}+{int(f_pk_float[0])}-{int(f_pk_int[-1])}+{int(f_pk_float[-1])}_calc.csv'
    with open(new_file_calc, 'w', newline='') as csvfile:
        writer = csv.writer(csvfile, delimiter=';')
        writer.writerow(f_pk_int)
        writer.writerow(f_pk_float)

        writer.writerow(f_mark_ditch)
        writer.writerow(f_mark_ditch_fact)

        writer.writerow(f_mark_pillow)
        writer.writerow(f_mark_pillow_fact)

        writer.writerow(f_mark_pipe)
        writer.writerow(f_mark_pipe_fact)

        writer.writerow(f_mark_filling)
        writer.writerow(f_mark_filling_fact)

        writer.writerow(f_mark_earth)
...
def file():
    ...
    data_fact = []
    with open(open_file) as r1_file:
        file1_reader = csv.reader(r1_file, delimiter=";")
        for row in file1_reader:
            data_fact.append(row)
    for i in range(len(data_fact[0])):
        pk_int.append(data_fact[0][i])
    for i in range(len(data_fact[1])):
        pk_float.append(data_fact[1][i])
    for i in range(len(data_fact[2])):
        mark_ditch.append(float(data_fact[2][i]))
    for i in range(len(data_fact[3])):
        mark_ditch_fact.append(float(data_fact[3][i]))
    for i in range(len(data_fact[4])):
        mark_pillow.append(float(data_fact[4][i]))
    for i in range(len(data_fact[5])):
        mark_pillow_fact.append(float(data_fact[5][i]))
    for i in range(len(data_fact[6])):
        mark_pipe.append(float(data_fact[6][i]))
    for i in range(len(data_fact[7])):
        mark_pipe_fact.append(float(data_fact[7][i]))
    for i in range(len(data_fact[8])):
        mark_filling.append(float(data_fact[8][i]))
    for i in range(len(data_fact[9])):
        mark_filling_fact.append(float(data_fact[9][i]))
    for i in range(len(data_fact[10])):
        mark_earth.append(float(data_fact[10][i]))

    # редактируем список pk_float, заменяем '0' на '00'
    for i in range(len(pk_float)):
        if pk_float[i] == '0':
            pk_float[i] = '00'
    name_file = f'           {data_fact[0][0]}+{data_fact[1][0]}0 - {data_fact[0][-1]}+{data_fact[1][-1]}         '
...
def calc():
    ...
    def basement_all_ditch():
        ...
        print(f'Длина списка {len(new_distance_pk)}')
        print(f'Длина списка - 1  {len(new_distance_pk)-1}')
        p = 304.00
